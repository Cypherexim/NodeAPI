...
exports.getphilipImport = async (req, res) => {
    //db.connect();
    try {
        ...
        if (check) {
            if (page != null && itemperpage != null) {
                await db.query(query.get_philip_import_pagination, [fromDate, toDate, HSCODE, HSCodeDesc, Importer_Name, EXPORTER_NAME, parseInt(itemperpage), (parseInt(page) - 1) * parseInt(itemperpage)], (error, results) => {
                    return res.status(200).json(success("Ok", results.rows, res.statusCode));
                })
            } else {
                await db.query(query.get_philip_import, [fromDate, toDate, HSCODE, HSCodeDesc, Importer_Name, EXPORTER_NAME], (error, results) => {
                    return res.status(200).json(success("Ok", results.rows, res.statusCode));
                })
            }
        } else {
